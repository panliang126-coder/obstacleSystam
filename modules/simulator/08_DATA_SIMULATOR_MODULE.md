...
- JSON Schema 校验的 `dynamic-crossing-v1` 场景；
- 按组件名从主种子派生的稳定随机流；
- 可手动推进且不读取墙钟的 `SimClock`；
- 与 `SensorDriver` 同构的基础雷达模拟器；
- 距离、方位、俯仰、径向速度、探测概率和可配置高斯噪声；
- 规范化 `sensor/1.0` 与 `health/1.0` 输出；
- 同场景、种子和版本的完整规范化事件哈希一致性测试。

该基线只覆盖基础雷达模型和 STEP/加速式执行，不声称已经满足本文件中的
EO/IR、GNSS/IMU、天气、通信、故障时间线、回放、1 小时实时和 2 倍峰值验收项。
